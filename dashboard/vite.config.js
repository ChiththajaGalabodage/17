...
const reportPath = fileURLToPath(new URL('../reports/report.json', import.meta.url))
const testCodePath = fileURLToPath(new URL('../tests/test_generated.py', import.meta.url))
...
function pythonExecutable() {
  const venvPython = fileURLToPath(new URL('../.venv/Scripts/python.exe', import.meta.url))
...
function runPipelineEndpoint(pathname) {
  return async (request, response) => {
    ...
    if (runState.running) {
      ...
      response.setHeader('Content-Type', 'application/json; charset=utf-8')
      response.end(JSON.stringify({ error: 'Pipeline is already running', runState }))
      return true
    }
    ...
    const child = spawn(pythonExecutable(), ['main.py', '--predictive-test-selection'], {
      cwd: repoRoot,
      windowsHide: true,
      env: {
        ...process.env,
    ...
    child.on('close', (code) => {
      ...
      runState.message = code === 0 ? 'Pipeline finished successfully' : 'Pipeline finished with errors'
